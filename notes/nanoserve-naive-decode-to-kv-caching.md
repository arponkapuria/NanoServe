# From Naive Decode to KV Cache: How LLM Serving Got 4x Faster With One Idea

I've been building a tiny LLM inference engine from scratch (NanoServe) — no vLLM, no SGLang, just PyTorch and a Qwen3-1.7B model running on a MacBook Air's GPU (MPS, not CUDA, so no shortcuts either). The goal is to actually understand what's happening under the hood every time we hit "generate" on a chatbot and later converting it into a production-grade mini llm inference engine.
...
```mermaid
flowchart TB
    subgraph Prefill["PREFILL — one shot, fully parallel"]
        direction LR
        P1["P1"] & P2["P2"] & P3["P3"] & P4["...Pn"] --> M1["model processes\nall n tokens at once"]
        M1 --> T1["first generated token"]
    end
    subgraph Decode["DECODE — sequential, one token at a time"]
        direction LR
        T1 --> D1["token 1"] --> D2["token 2"] --> D3["token 3"] --> D4["..."]
    end
```
...
```mermaid
flowchart LR
    A["Step 1: [P1..Pn]"] -->|"forward pass over n tokens"| B["Step 2: [P1..Pn, T1]"]
    B -->|"forward pass over n+1 tokens"| C["Step 3: [P1..Pn, T1, T2]"]
    C -->|"forward pass over n+2 tokens"| D["... recomputes everything, every time"]
```
...
```mermaid
flowchart LR
    A["Prefill: full prompt\n(one forward pass)"] -->|"cache K,V for every prompt token"| B["Cache: [K1,V1]...[Kn,Vn]"]
    B -->|"step 1: only compute K,V for new token"| C["+ [Kn+1,Vn+1]"]
    C -->|"step 2: only compute K,V for new token"| D["+ [Kn+2,Vn+2]"]
    D -->|"..."| E["cache grows by 1 token per step,\nnever recomputed"]
```
...
Here's the same benchmark, same prompt, same hardware, Naive decode vs. KV cache:
